...
public interface NotificationsRepository extends JpaRepository<Notifications,String> {
	@Query("SELECT n FROM Notifications n WHERE (n.targetUser = :user OR n.targetType = 'GENERAL') AND n.createdAt <= :userCreatedAt")
	Page<Notifications> findUserNotifications(@Param("user") User user,@Param("userCreatedAt") Date userCreatedAt,Pageable pageable);

	@Query("SELECT n FROM Notifications n WHERE (n.targetUser = :user OR n.targetType = 'GENERAL') AND n.messageRead IS NULL AND n.createdAt <= :userCreatedAt ")
